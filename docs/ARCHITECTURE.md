...
- Auth: cookie session (`HUB_USERS_JSON` + `HUB_SESSION_SECRET`); cloud fail-closed
...
| `fetch_post_store` | fetch_post_jobs.json |
| `auto_follow_store` | auto_follow.json |
...
- Hub caption generation: **local templates** (`src/hub/publish_caption.py`); optional EN polish if key set
- LINE bot: `line_bot/openai_reply.py`, `ops_analyst.py`

## Co-Agent

- Public catalog: `GET /api/co/catalog` (slim fields via `src/hub/co_catalog.py`)
- Authenticated full catalog: `GET /api/hub/catalog`
- Traffic: `data/co_traffic/`

## Identity model (Phase B)

- **`property_id` (UUID):** canonical unique internal identity
- **`property_code` (e.g. PTPxxxx):** human-facing reference; **not guaranteed unique**
- Resolution: `src/hub/property_resolve.py` — duplicate codes return ambiguous, never silent first-match
- Public projection: `src/hub/public_projection.py` — explicit field allowlist for preview/Co-Agent

See `docs/DUPLICATE-CODE-TRIAGE.md` and `docs/PROPERTY-IDENTITY.md`.

## Deployment

- `Dockerfile` → `scripts/docker_entrypoint.sh` → `hub_server.py`
- Volume mount: `/app/data`
- Image seeds from `data_seed/` (synthetic) via non-destructive `cp -an`
- **Exactly 1 Fly machine** (split-brain if >1)

See `DEPLOY_FLY.md`, `docs/OPERATIONS-SAFETY.md`, and `docs/PHASE-C-OPERATIONS.md`.
