#!/usr/bin/env python3
"""Phase Z6 authenticated QA — lease capture + freshness + opportunities."""
...
OUT = Path("/tmp/pantip-phase-z6-visual")
...
def wait_server(timeout: float = 20.0) -> bool:
    ...
    while time.time() - start < timeout:
        try:
            with urllib.request.urlopen(f"{BASE}/listing-freshness/", timeout=2) as r:
                return r.status == 200
        except Exception:
            time.sleep(0.3)
    return False
...
def main() -> int:
    ...
    proc = subprocess.Popen(
        ["bash", str(ROOT / "scripts/start_z6_operational_pilot.sh")],
        cwd=str(ROOT),
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            pages = [
                ("lease-opportunity", "/lease-opportunities/", 1440, 900),
        ...
        if result["login_ok"] and len(result["screenshots"]) >= 4:
            result["gate"] = "PASS"
